...
def handle_zip(myblob: func.InputStream, storage: Storage):
    regex = re.compile("^.*(\/)?\._.*")
    # I wanted to load the zip file in memory rather than extracting it in the filesystem.
    zip_blob = BytesIO(myblob.read())
    zip_file = ZipFile(zip_blob)
    contents = zip_file.namelist()
    contents_filtered = [
        item for item in contents if item not in list(filter(regex.search, contents))
    ]
